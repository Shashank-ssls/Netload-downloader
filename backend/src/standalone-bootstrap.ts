 ...
 *
 * When running as a PACKAGED executable (netload.exe), point every data + binary dir
 * at the portable folder that holds the exe, so the whole thing is self-contained and
 * writes downloads/cookies/db next to itself rather than into some random CWD. In a
 * normal dev run (unpackaged) this is a no-op — config's repo-relative defaults apply.
 *
